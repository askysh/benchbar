...
fl_report_redact_file() {
  ...
  #    INI (key = value), shell (key=value) and header (Key: value) forms. The
  #    last one masks to the end of the line, or to the next quote or comma
  #    when the key sits inside a one line JSON document.
