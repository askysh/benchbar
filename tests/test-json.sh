...
assert_eq "$VER" "$(printf '%s' "$OUT" | jget - 'd["cli_version"]')"
...
assert_eq "1 ${VER} com.benchbar.frappe-bench http://macdev:8000" \
...
import json, sys
d = json.load(sys.stdin)
assert d["schema_version"] == 1 and d["cli_version"] == "'"$VER"'", d
